...
static int wait_value(u32 addr, u32 mask, u32 value)
{
	u32 count;
...
static void set_field(u32 addr, u8 shift, u8 width, u32 value)
{
	u32 mask;
	u32 reg;
...
static void uart_puts_at(u32 base, const char *text)
{
	while (*text)
		uart_putc_at(base, (u8)*text++);
}

/* IRUN crumbs: suppressed when IPL config verbose is on (pretty boot logs). */
static int ipl_quiet;

static void ipl_crumb(char ch)
{
	if (!ipl_quiet)
...
/* Drop pending RX bytes so leftover stage-1 overflow cannot desync GET. */
...
static void pll_program(u32 addr, u32 value)
{
...
static void apply_patches(const struct field_patch *patches, u32 count)
{
	u32 i;
...
static int efuse_read(u32 address, u8 *value)
{
	u32 count;
	u32 cmd;
...
static void run_uart_payload(u8 *buf, u32 size)
{
	...
	/*
	 * Raw U-Boot: already received in place at UBOOT_ENTRY (see
	 * ipl_post_mmu).  Do not bounce via STAGE_BUF — a second  copy of a
	 * multi‑hundred‑KiB image delayed or broke the handoff.
	 */
...
void ipl_post_mmu(void)
{
	...
	if (!(cfg.flags & IPL_CFG_SKIP_SPI)) {
		if (try_spi_bootcode(&cfg) == 0)
			return;
	}

uart_load:
	/*
	 * Receive directly to the U-Boot link address so --full --uboot matches
	 * the proven pre-bootcode IPL behaviour.  GXBC-wrapped bootcode is
	 * detected after the transfer and relocated.
	 */
